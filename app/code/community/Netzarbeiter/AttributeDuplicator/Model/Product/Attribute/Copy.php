...
class Netzarbeiter_AttributeDuplicator_Model_Product_Attribute_Copy
{
    ...
    public function copy($sourceAttributeCode, $targetAttributeCode, $targetLabel = '')
    {
        ...
        if (!$source || !$source->getId()) {
            Mage::throwException(Mage::helper('attributecopy')->__(
                'Invalid source attribute code "%s"', $sourceAttributeCode
            ));
        }
        ...
        $newAttribute = Mage::getModel('catalog/entity_attribute')
            ->loadByCode($this->_entityType, $targetAttributeCode);
    ...
    protected function _validateTargetCode($targetAttributeCode)
    {
        ...
        if (strtolower($targetAttributeCode) != $targetAttributeCode) {
            Mage::throwException($helper->__(
                'Invalid target attribute code "%s" must be lower case', $targetAttributeCode
            ));
        }

        if (!preg_match('/^[a-z][a-z0-9_]+$/', $targetAttributeCode)) {
            Mage::throwException($helper->__(
                'Invalid target attribute code "%s" must contain only characters and underscores', $targetAttributeCode
            ));
        }
        ...
        if ($target && $target->getId()) {
            Mage::throwException($helper->__(
                'Invalid target attribute code "%s" already exists', $targetAttributeCode
            ));
        }
    ...
    {
        $postfix = ' Copy';
        if (!preg_match('/(' . $postfix . '(?: ([0-9]+))?)$/', $oldLabel, $match)) {
            $newLabel = $oldLabel . $postfix;
        } else {
            $counter = isset($match[2]) ? intval($match[2]) + 1 : 2;
            $newLabel = preg_replace('/' . $match[1] . '$/', $postfix . ' ' . $counter, $oldLabel);
        }
        return $newLabel;
    }
     ...
     */
    protected function _copyAttributeSetInfo(
        Mage_Eav_Model_Entity_Attribute $source, Mage_Eav_Model_Entity_Attribute $target
    ) {
